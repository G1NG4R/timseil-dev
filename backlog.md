...
| Datum | Aus Phase | Was | Status |
|---|---|---|---|
| 2026-08-24 | E2 | **Dependabots Grenze lag auf `major`, die Politik dieses Repositories liegt bei Go und Prometheus auf `minor`.** Zwei rote PRs am Montagmorgen haben den Unterschied sichtbar gemacht. #174 hob `golang:1.26-alpine` auf `1.27-alpine` und war rot, bevor jemand ihn gelesen hatte: `tools/check-versions.sh` hält `go.mod` gegen das Tag, und das `docker`-Ökosystem darf `go.mod` nicht anfassen — der PR konnte per Konstruktion nie grün werden. #176 hob Prometheus auf `v3.14.0` und verließ damit die `3.13.x`-LTS-Linie, die ADR 0007 ausdrücklich wählt. Beide Male war die Entscheidung im Repository längst getroffen und aufgeschrieben, nur die Automatik kannte sie nicht. **Der zweite Fund steckt in #176 und ist der allgemeinere:** jeder Bump, der eine von `stack.yaml` verfolgte Major.Minor bewegt, macht `stack.gen.json` stale, und Dependabot kann kein `make gen` laufen lassen — der nächste Postgres-Minor trifft das genauso, und dort *will* man den Bump. Gemessen statt vermutet, `genstack` gegen eine Kopie gefahren: `v3.13.2 → v3.13.3` lässt die Datei unberührt, `v3.14.0` ändert sie. | **erledigt im Repository** — zwei `ignore`-Regeln auf den Namen statt auf `*`, weil eine breite Minor-Sperre auf `docker-compose` die Postgres-Security-Minors mitgeblockt hätte; beide PRs geschlossen. Die stale-Klasse ist **nicht** gelöst, sondern als Handgriff im Kopf von `.github/dependabot.yml` beschrieben |
...
| 2026-08-23 | F4 | **Die tatsächliche Kadenz der Sonde ist nicht belegt, nur ihr Anlaufen.** Der Cron steht auf `3-58/5`, also zwölf Läufe je Stunde. Bis 15:04 UTC gab es seit 14:56 **einen** geplanten Lauf, wo zwei fällig gewesen wären. GitHub verzögert geplante Workflows und lässt sie unter Last auch aus — beides bekannt und beides unbeeinflussbar. **Das zählt:** `down_sec` ist fehlgeschlagene Checks **mal `ProbeInterval`**, und wenn real seltener gemessen wird als die Konstante behauptet, ist jede Ausfalldauer zu klein. `check-probe-cadence` prüft die zwei *deklarierten* Hälften, nicht die gefahrene. **Nachgezählt am 24.08.2026, und die Abweichung ist keine Nuance: 41 Läufe in 23,66 h statt 284.** Das sind **14 %** der deklarierten Kadenz, ein echter Abstand von **35 Minuten** statt fünf — Faktor 6,9. Damit ist die Folge nicht mehr theoretisch: `down_sec` ist fehlgeschlagene Checks **mal 300 s** (`ops.go:55`), also wäre **jede Ausfalldauer auf der Seite rund siebenmal zu klein**. Ein Ausfall von 35 Minuten, den genau eine Sonde trifft, erschiene als 5 Minuten. Das ist Invariante 1 in Betriebskleidung: keine erfundene Zahl, sondern eine systematisch geschönte. **Die Konstante zu drehen wäre falsch** — der echte Abstand schwankt, GitHub verzögert und lässt aus. Der Weg ist, `down_sec` aus den tatsächlichen Zeitstempeln zu bilden statt aus Anzahl mal Konstante. `check-probe-cadence` prüft die zwei *deklarierten* Hälften und kann das prinzipbedingt nicht sehen. | **offen und wichtig** — gemessen, Entwurf offen |
