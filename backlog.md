...
| 29.08.2026 | G6 | **Die neunte Behauptung ist vier Phasen lang niemandem aufgefallen.** `check-deployed` stellt acht Behauptungen von einem fremden Rechner aus und sagt selbst, dass eine fehlt: ob die **laufenden** Container die veröffentlichten Digests sind, sieht nur der Host. In Stufe F wurde sie gestellt — der Backlog trägt dort zweimal „`make check-deployed --host`: 10 von 10 Behauptungen". Seit G3 steht in jeder Abnahme stattdessen der Satz „`--host` ist nicht gefahren", in G3, G4, G5 und G6, **ohne dass irgendwo entschieden worden wäre, sie fallen zu lassen.** Das ist die teurere Sorte Drift: kein Test wurde rot, kein Kriterium wurde geändert, eine Gewohnheit hat einfach aufgehört. Der Anlass, es jetzt zu bemerken, ist banal — vier gleichlautende Sätze untereinander, als der G6-Text neben dem G5-Text stand. **Aufgabe:** den Lauf in die nächste Abnahme zurückholen und dort belegen, dass beide Digests übereinstimmen. **Abnahmekriterium:** 10 von 10 statt 8 von 9, für `api` und `web`. **Zu entscheiden ist dabei, ob es jede Phase sein muss** — acht von hier sind billig und decken das meiste ab, die zwei vom Host kosten einen zweiten Weg. Eine Antwort wie „jede Stufe, nicht jede Phase" wäre auch eine, aber sie muss getroffen und aufgeschrieben werden, statt weiter zu unterbleiben. Das Ergebnis eines Laufs gehört nach `backlog.local.md`, die Aufgabe hierher. | offen |

| 29.08.2026 | G6-Abnahme | **Der überlappende Start aus ADR 0035, zufällig in zwei aufeinanderfolgenden Anfragen mitgemessen.** Beim Warten auf den Deploy von `4fe6f84` fragte die Schleife `/api/health` zweimal im Abstand von Millisekunden. Die erste Antwort trug `sha 4fe6f84`, die zweite `version v0.14.0-1-gcd4ec47` — **das alte Binary** — und in derselben zweiten Antwort stand `lastDeploy.sha = 4fe6f84`. Also: der alte Container beantwortete die Anfrage und las dabei die **neue** Deploy-Zeile aus der Datenbank, weil die Datenbank beiden gehört und der Rollout sie vor dem Containertausch schreibt. Zwanzig Sekunden später waren fünf Abfragen in Folge einig: `v0.14.0-2-g4fe6f84`, `startedAt 19:32:21`. **Wert des Fundes:** ADR 0035 begründet damit, dass `lib/api/health.ts` die Felder defensiv liest statt dem generierten Typ zu glauben — hier ist der Beleg, dass das Fenster wirklich existiert und nicht nur beschrieben ist. Und die Messlehre daneben: **zwei `curl` sind zwei Anfragen.** Wer die Prüfung und die Ausgabe in getrennten Aufrufen macht, kann über zwei verschiedene Maschinen berichten und es für einen Widerspruch halten. Eine Antwort holen, dann alles daraus lesen. | erledigt |
