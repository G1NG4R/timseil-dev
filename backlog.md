...
| 2026-08-20 | E1 | Der `images`-Job blockiert den Merge und dauert am längsten. Wenn er stört: auf `paths:`-Filter einschränken (nur bei Änderungen an `compose.yaml`, den Dockerfiles, `ops/postgres/initdb/`). Bewusst **nicht** gebaut — ein Pfadfilter ist eine Behauptung darüber, was ein Image beeinflusst, und die ist leichter falsch als eine Wartezeit. | offen |
