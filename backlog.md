...
| 2026-08-21 | E2a | **Nichts hält `api/go.mod` gegen das Build-Image.** `go.mod` sagte `go 1.26.0`, `api/Dockerfile` baut aus `golang:1.26-alpine` (= 1.26.6). `setup-go` liest `go.mod`, also testete die Pipeline mit einer anderen — und mit 21 erreichbaren stdlib-Schwachstellen behafteten — Toolchain als der, die das ausgelieferte Binary baut. Von govulncheck im ersten CI-Lauf gefunden, lokal unsichtbar, weil die Entwickler-Toolchain schon 1.26.6 war. | erledigt — `tools/check-versions.sh` hält jetzt beide Paare gegeneinander |
| 2026-08-21 | E2a | **Dependabots erste Welle bot sofort drei verbotene Majors an**: `node 24→26` (CLAUDE.md Zeile 54 und #46 verbieten es bis 2026-10-28) sowie `@types/node ^20→^26`, `eslint ^9→^10`, `typescript ^5→^7`. Ursache: `allow: direct` steuert *welche* Abhängigkeiten, nicht *welche Art* Update. | behoben — `ignore` auf Majors für npm, Docker und Go; Actions behalten ihre |
...
| 2026-08-21 | E2a | ~~CodeQL gegen das 5-Minuten-Budget~~ — **gemessen und entschieden:** Run 32493542206, Wall-Clock 2:15, beide CodeQL-Sprachen liegen unter dem `images`-Job und kosten null Feedback-Zeit. Bleibt auf `pull_request`. | erledigt, ADR 0031 §Belege |
