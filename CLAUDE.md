...
- **Der Squash schreibt sie klein: `Co-authored-by`.** GitHub normalisiert den
  Trailer beim Anhängen. Wer prüfen will, ob sie angekommen ist, sucht
  **case-insensitiv** — `grep -ci 'co-authored-by'`. Ein verankertes
  `grep -c '^Co-Authored-By:'` zählt auf jedem Squash null und sieht aus wie ein
  Befund. Passiert am 25.09.2026, und die falsche Zahl stand danach in einer
  Abnahme, in einem ADR-Nachtrag und in dieser Datei. Die Doppelung, vor der die
  Regel oben warnt, steht seitdem auf `a2684275`.
