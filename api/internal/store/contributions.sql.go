...
const getContributions = `-- name: GetContributions :one
...
type GetContributionsRow struct {
...
func (q *Queries) GetContributions(ctx context.Context, login string) (GetContributionsRow, error) {
	row := q.db.QueryRow(ctx, getContributions, login)
	var i GetContributionsRow
	err := row.Scan(
...
const upsertContributions = `-- name: UpsertContributions :exec
...
type UpsertContributionsParams struct {
	Login              string
	TotalContributions int32
	Weeks              []byte
}
...
func (q *Queries) UpsertContributions(ctx context.Context, arg UpsertContributionsParams) error {
	_, err := q.db.Exec(ctx, upsertContributions, arg.Login, arg.TotalContributions, arg.Weeks)
	return err
}
